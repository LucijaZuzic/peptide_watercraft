...
round_val = ["R2", "MAE", "RMSE", "MSE", "euclid", "haversine"]

df_dictio = pd.read_csv("data_frame_traj_val_merged.csv", index_col = False)
...
for test_var in ["wilcoxon", "mann_whitney"]:
    for metric in dictio[var][model][ws]:
        if test_var == "wilcoxon":
            dicti_wilcoxon_traj_new = {"variable": [], "ws": [], "model1": [], "model2": [], "u": [], "p": []}
            for var in var_list:
                for ws in ws_list:
                    for model1_ix in range(len(model_list)):
                        ...
                        for model2_ix in range(model1_ix + 1, len(model_list)):
                            ...
                            dicti_wilcoxon_traj_new["ws"].append(ws)
                            dicti_wilcoxon_traj_new["model1"].append(model1)
                            dicti_wilcoxon_traj_new["model2"].append(model2)
                            dicti_wilcoxon_traj_new["u"].append(uval)
                            dicti_wilcoxon_traj_new["p"].append(pval)
            df_write = pd.DataFrame(dicti_wilcoxon_traj_new)
            df_write.to_csv("dicti_wilcoxon_traj_" + metric + ".csv", index = False)
