...
vehicle_zero = os.listdir("csv_results_mask/1/1/")[0]
ride_zero = os.listdir("csv_results_mask/1/1/" + vehicle_zero)[0]
var_list = os.listdir("csv_results_mask/1/1/" + vehicle_zero + "/" + ride_zero + "/")
model_list = os.listdir("csv_results_mask/1/1/" + vehicle_zero + "/" + ride_zero + "/" + var_list[0] + "/")

for nf2 in range(sf2):
    for nf1 in range(sf1):
        for model in model_list:
            for ws in [2, 3, 4, 5, 10, 20, 30]:
                for vehicle in os.listdir("csv_results_mask/" + str(nf1 + 1) + "/" + str(nf2 + 1) + "/"):
                    for ride in os.listdir("csv_results_mask/" + str(nf1 + 1) + "/" + str(nf2 + 1) + "/" + vehicle + "/"):
                        kfile = vehicle + "/cleaned_csv/" + ride + ".csv"
                        pd_file_dict = dict()
                        for var in var_list:
                            path_to_file = "csv_results_mask/" + str(nf1 + 1) + "/" + str(nf2 + 1) + "/" + vehicle + "/" + str(ride) + "/" + var + "/" + model + "/" + str(ws) + "_predictions.csv"
                        ...
                        for ix in range(len(pd_file_dict["longitude_no_abs"]["predicted_total"])):
                            actual_trajs["no abs"]["predicted_long"].append(actual_trajs["no abs"]["predicted_long"][-1] + pd_file_dict["longitude_no_abs"]["predicted_total"][ix])
                            actual_trajs["no abs"]["predicted_lat"].append(actual_trajs["no abs"]["predicted_lat"][-1] + pd_file_dict["latitude_no_abs"]["predicted_total"][ix])
                        ...
                        for m in actual_trajs:
                            ...
                            path_to_file = "csv_results_mask_traj/" + str(nf1 + 1) + "/" + str(nf2 + 1) + "/" + vehicle + "/" + str(ride) + "/" + m + "/" + model + "/"
