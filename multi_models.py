...
def _create_seq_model(
    ...
    if conv1_filters > 0:
        ...
        x = Bidirectional(LSTM(num_cells))(model_input)
...
def multi_seq_model(
    ...
    for i in range(num_props):
        input1, output1 = _create_seq_model(final_size, input_shape, conv1_filters, conv2_filters, conv_kernel_size, num_cells, dropout)
        inputs.append(input1)
        outputs.append(output1)

    return Model(inputs=inputs, outputs=outputs)

def _one_prop_model(final_size, lstm1=5, lstm2=5, dense=15, dropout=0.5, lambda2=0.0):
    # LSTM model which processes dipeptide AP scores.

    input_layer = Input((None, 1))
    lstm_layer_1 = Bidirectional(LSTM(lstm1, return_sequences=True))(input_layer)
...
def only_amino_di_tri_model(num_props, final_size, lstm1=5, lstm2=5, dense=15, dropout=0.5, lambda2=0.0):
    # Instantiate separate submodels.
    inputs = []
    outputs = []
    for i in range(num_props):
        input1, output1 = _one_prop_model(final_size, lstm1, lstm2, dense, dropout, lambda2)
        inputs.append(input1)
        outputs.append(output1)
